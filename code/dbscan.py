...
drop_df = df[df['ltecell_name'] == cell].drop(['timestamp', 'datetime', 'ltecell_name', 'time_advance_avg', 'traffic_volume_data', 'user_throughput_dl_avg', 'cell_throughput_ul_avg', 'prb_dl_usage_rate_avg', 'daytime'], axis=1)
num_cols = ['users_avg', 'prb_ul_usage_rate_avg', 
            'hsr_intra_freq', 'hsr_inter_freq',
            'cell_throughput_dl_avg', 'user_throughput_ul_avg',
            'tp_carrier_aggr', 'carrier_aggr_usage', 'cqi_avg']
# %%
...
# %%
clusters = DBSCAN(eps = knee, min_samples = 2*10).fit(scaled_df)
# get cluster labels
# %%
